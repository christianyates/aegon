 ...
 */
function aegon_preprocess_html(&$vars) {
  $vars['classes_array'][] = 'theme-markup-2';
  $vars['classes_array'][] = _aegon_get_layout();
}

/**
 * Implements hook_html_head_alter().
 */
function aegon_html_head_alter(&$head_elements) {
...
function aegon_preprocess_page(&$variables) {
 ...
 */
function aegon_preprocess_block(&$vars) {
 ...
 */
function _aegon_variable_get($key, $default) {
  global $theme;
 ...
 */
function _aegon_get_layout() {
  $layout_patterns = _aegon_variable_get('layout', array('<global>' => 'body-layout-fixed-ca'));
 ...
 */
function aegon_node_view_alter(&$build) {
 ...
 */
function aegon_preprocess_forum_topic_list(&$vars) {
 ...
 */
function aegon_preprocess_media_gallery_license(&$vars) {
...
/**
 * Implement custom menu output that simplifies markup to work correctly with Aegon's responsive code.
 */
function aegon_links__system_main_menu($variables) {
  $html = '';
